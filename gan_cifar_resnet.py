...
DATA_DIR = '/newNAS/Workspaces/CVGroup/mkxu/cifar-10-python'
if len(DATA_DIR) == 0:
...
MODE = 'swgan-al'
# Valid options are: wgan-gp, swgan-gp, sgan-gp, wgan-al, swgan-al, sgan-al
SAMPLE_SIZE = 8 # How much gradients sampled between each pair of real data and fake data
LAMBDA = 10 # Gradient penalty lambda hyperparameter in Quadratic Penalty Method
RHO = 1e-5 # Quadratic weight penalty rho hyperparameter in Augmented Lagrangian Method
...
INCEPTION_FREQUENCY = 4000 # How frequently to calculate Inception score
...
with tf.Session() as session:
    ...
    all_real_labels = tf.placeholder(tf.float32, shape=[BATCH_SIZE])
    ...
    for i, device in enumerate(DEVICES_B):
        with tf.device(device):
            ...
            labels_deep = tf.tile(labels, [SAMPLE_SIZE])
            alpha = tf.random_uniform(
                shape=[BATCH_SIZE/len(DEVICES_A)*SAMPLE_SIZE, 1],
                minval=0.,
                maxval=1.
            )
            differences = fake_data_deep - real_data_deep
            interpolates = real_data_deep + (alpha*differences)
            gradients = tf.gradients(Discriminator(interpolates, labels_deep)[0], [interpolates])[0]
            
            if 'swgan' in MODE:
                gradients = tf.reshape(gradients, [BATCH_SIZE / len(DEVICES_A), SAMPLE_SIZE, OUTPUT_DIM])
    ...
    def get_score(n=50000, noise=None, split_std=False):
        ...
        if ref_icp_activations is None:
            dataX, dataY, testX, testY = load_cifar10()
        ...
        icp_preds, icp_activcations = icp_model.get_preds(all_samples)
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(N_CRITIC):
            ...
            if 'al' in MODE:
                if CONDITIONAL and ACGAN:
                    _disc_cost, _disc_wgan, _disc_acgan, _disc_acgan_acc, _disc_acgan_fake_acc, _, __ = session.run([disc_cost, disc_wgan, disc_acgan, disc_acgan_acc, disc_acgan_fake_acc, disc_train_op, alpha_train_op], feed_dict={all_real_data_int: _data, all_real_labels:_labels, _iteration:iteration})
                else:
                    _disc_cost, _, __ = session.run([disc_cost, disc_train_op, alpha_train_op], feed_dict={all_real_data_int: _data, all_real_labels:_labels, _iteration:iteration})
            else:
                if CONDITIONAL and ACGAN:
        ...
        if iteration % INCEPTION_FREQUENCY == INCEPTION_FREQUENCY-1:
            if iteration != 99999 and iteration != 49999:
                icp_score, icp_mean, icp_std, fid, fid_mean, fid_std = get_score(50000)
                lib.plot.plot('icp_score_50k', icp_score)
                lib.plot.plot('fid_50k', fid)
            else:
                icp_score, icp_mean, icp_std, fid, fid_mean, fid_std = get_score(50000, split_std=True)
                lib.plot.plot('icp_score_50k', icp_score)
                lib.plot.plot('fid_50k', fid)
                lib.plot.plot('icp_score_50k_std', icp_std)
                lib.plot.plot('fid_50k_std', fid_std)
